...
int MJobAllocMNL(
  ...
  DBG(4,fSCHED) DPrint("%s(%s,MFeasibleList,NodeMap,%s,%s,%ld)\n",
    FName,
    (J != NULL) ? J->Name : "NULL",
  ...
  if (J->Flags & (1 << mjfHostList))
        ...
        if (J->ReqHList[nindex].N == MNodeList[0][index].N)
          ...
          if (J->ReqHLMode == mhlmSubset)
            ...
            if (J->ReqHLMode != mhlmSuperset)
              ...
              MNodeList[0][index].TC =
                MIN(MNodeList[0][index].TC,J->ReqHList[nindex].TC);
              }

            TC += MIN(MNodeList[0][index].TC,J->ReqHList[nindex].TC);
            }
    ...
    if (((TC < J->Request.TC) && 
        !(J->Flags & (1 << mjfBestEffort)) &&
         (J->ReqHLMode != mhlmSubset)) || 
         (TC == 0))
      {
      DBG(2,fSCHED) DPrint("WARNING:  inadequate tasks specified in hostlist for job %s (%d < %d)\n",
        J->Name,
        TC,
        J->Request.TC);
    ...
    if (J->ReqHLMode != mhlmSubset)
      ...
      for (nindex = 0;J->ReqHList[nindex].N != NULL;nindex++)
        ...
        if (Delta > 0)
          ...
          if (MOutList != NULL)
            memcpy(&MOutList[0][rnindex],&RQ->NodeList[rnindex],sizeof(mnalloc_t));
      ...
      if (rnindex == 0)
        {
        DBG(3,fSCHED) DPrint("WARNING:  empty hostlist for job %s in %s()\n",
          J->Name,
          FName);
      ...
      if (MOutList != NULL)
        MOutList[0][rnindex].N = NULL;

      if (TC > 0)
        {
        DBG(3,fSCHED) DPrint("WARNING:  cannot allocate tasks specified in hostlist for job %s (%d remain)\n",
          J->Name,
          TC);
      ...
      DBG(3,fSCHED) DPrint("INFO:     %d requested hostlist tasks allocated for job %s (%d remain)\n",
        J->Request.TC,
        J->Name,
        TC);
  ...
  for (rqindex = 0;J->Req[rqindex] != NULL;rqindex++)
    ...
    if (FeasibleTaskCount[rqindex] < RQ->TaskCount)
      {
      DBG(0,fSCHED) DPrint("ERROR:    invalid nodelist for job %s:%d (inadequate taskcount, %d < %d)\n",
        J->Name,
        rqindex,
    ...
    if ((RQ->NodeCount > 0) &&
      ...
      {
      DBG(0,fSCHED) DPrint("ERROR:    invalid nodelist for job %s:%d (inadequate nodecount, %d < %d)\n",
        J->Name,
        rqindex,
  ...
  for (nsindex = 0;nsindex < 2;nsindex++)
    ...
    for (rqindex = 0;J->Req[rqindex] != NULL;rqindex++)
      ...
      if (((tmpRSS == mrssNONE) && (nsindex == 0)) || 
        ...
        DBG(7,fSCHED) DPrint("INFO:     ignoring pass 1 for job %s:%d (node set forced in feasible list)\n",
          J->Name,
          rqindex);

        continue;
        }
      else if ((tmpRSS != mrssNONE) &&
               (nsindex == 1)) 
        {
      ...
      switch (tmpNAPolicy)
        ...
        case mnalCPULoad:
          ...
          while (1)
            ...
            for (nindex = 0;NodeList[nindex].N != NULL;nindex++)
              {
              NIndex = NodeList[FeasibleNodeCount[rqindex] - nindex - 1].N->Index;
            ...
            for (nindex = 0;NodeList[nindex].N != NULL;nindex++)
              {
              N = NodeList[FeasibleNodeCount[rqindex] - nindex - 1].N;
              ...
              NTasks = NodeList[FeasibleNodeCount[rqindex] - nindex - 1].TC;
              ...
              BestList[rqindex][NodeIndex[rqindex]].TC = NTasks;
        ...
        case mnalLastAvailable:
          ...
          for (nindex = 0;NodeList[nindex].N != NULL;nindex++)
            ...
            NodeList[FeasibleNodeCount[rqindex] - nindex - 1].TC -= TC;

            if (NodeList[FeasibleNodeCount[rqindex] - nindex - 1].TC <= 0)
              { 
              NodeMap[N->Index] = nmUnavailable;
              }
        ...
        case mnalMinResource:
          ...
          for (mem = 64;mem <= 8192;mem <<= 1)
            ...
            for (nindex = 0;NodeList[nindex].N != NULL;nindex++)
              ...
              if ((MNode[N->Index]->CRes.Mem > mem) && (mem < 8192))
                {
        ...
        default:
          ...
          DBG(1,fSCHED) DPrint("ERROR:    invalid allocation policy (%d) in %s()\n",
            tmpNAPolicy,
            FName);
...
/*
        fprintf(stderr,"NOTE:  cannot locate all required tasks for job %s in nodeset iteration %d (TaskCount: %d)\n",
          J->Name,
          nsindex,
          FeasibleTaskCount[rqindex]);
*/
  ...
  for (rqindex = 0;J->Req[rqindex] != NULL;rqindex++)
    ...
    if (TaskCount[rqindex] < RQ->TaskCount)
      {
      DBG(2,fSCHED) DPrint("ALERT:    inadequate tasks to allocate to job %s:%d (%d < %d)\n",
        J->Name,
        rqindex,
        TaskCount[rqindex],
        RQ->TaskCount);
    ...
    if ((RQ->NodeCount > 0) && 
        (NodeCount[rqindex] < RQ->NodeCount))
      {
      DBG(2,fSCHED) DPrint("ALERT:    inadequate nodes found for job %s:%d (%d < %d)\n",
        J->Name,
        rqindex,
        NodeCount[rqindex],
        RQ->NodeCount);
  ...
  DBG(2,fSCHED) DPrint("INFO:     tasks located for job %s:  %d of %d required (%d feasible)\n",
    J->Name,
    TotalAllocTaskCount,
    J->Request.TC,
    ...
    for (rqindex = 0;J->Req[rqindex] != NULL;rqindex++)
      ...
      for (index = 0;BestList[rqindex][index].N != NULL;index++)
        ...
        DBG(4,fSCHED) DPrint("INFO:     allocated MNode[%03d]x%d '%s' to %s:%d\n",
          index,
          BestList[rqindex][index].TC,
          BestList[rqindex][index].N->Name,
          J->Name,
          rqindex);
        }
      }    /* END for (rqindex) */
...
int MJobSelectMNL(
  ...
  DBG(4,fSCHED) DPrint("%s(%s,%s,%s,MNodeList,NodeMap,MaxSpeed,%lu)\n",
    FName,
    (J !=NULL) ? J->Name : "NULL",
    (P != NULL) ? P->Name : "NULL",
    (FNL == NULL) ? "NULL" : "FNL",
  ...
  /* NOTE:  overcommit preemption requires externally signalling that preemption *
  ...
  for (rqindex = 0;J->Req[rqindex] != NULL;rqindex++)
    ...
    if (MReqGetFNL(
      ...
      DBG(4,fSCHED) DPrint("INFO:     cannot locate adequate feasible tasks for job %s:%d\n",
        J->Name,
        rqindex);
    ...
    if (ITC < RQ->TaskCount)
      {
      DBG(4,fSCHED) DPrint("INFO:     insufficient idle tasks in partition %s for %s:%d: (%d of %d available)\n",
        P->Name,
        J->Name,
        RQ->Index,
        ITC,
      ...
      {
      DBG(4,fSCHED) DPrint("INFO:     insufficient idle nodes in partition %s for req %d: (%d of %d available)\n",
        P->Name,
        RQ->Index,
        INC,
        RQ->NodeCount);
      ...
      {
      DBG(7,fSCHED) DPrint("INFO:     adequate idle nodes/tasks located\n");
      }
    ...
    if (MNodeSelectIdleTasks(
         ...
         Reason) == FAILURE)
      {
      DBG(4,fSCHED) DPrint("INFO:     insufficient idle resources selected in partition %s: (%d of %d tasks/%d of %d nodes)\n",
        P->Name,
        TotalAvailITC,
        J->Request.TC,
        TotalAvailINC,
  ...
  if ((J->Flags & (1 << mjfHostList)) && 
    ...
    for (hindex = 0;J->ReqHList[hindex].N != NULL;hindex++)
      ...
      for (rqindex = 0;J->Req[rqindex] != NULL;rqindex++)
        {
        for (nindex = 0;LocalFNL[rqindex][nindex].N != NULL;nindex++)
          {
  ...
  if (PreemptNodesRequired == TRUE) 
    ...
    if (MSched.PreemptPolicy == mppOvercommit)
      ...
      TotalAvailINC += 1;
      TotalAvailIPC += RQ->TaskCount * ((RQ->DRes.Procs == -1) ? N->CRes.Procs : RQ->DRes.Procs);
      ...
      if (J->Req[1] == NULL) 
        {
        if (TotalJobProcs > 
           (P->ARes.Procs + PTC * RQ->DRes.Procs))
          {
      ...
      if (MJobReserve(J,jrPriority) == SUCCESS)
        {
        DBG(1,fALL) DPrint("NOTE:     job %s would start in %s without preemption (PC: %d)\n",
          J->Name,
  ...
  DBG(4,fSCHED) DPrint("INFO:     %d(%d) tasks/%d(%d) nodes found for job %s in %s\n",
    TotalAvailITC,
    TotalAvailPTC, 
    TotalAvailINC,
    TotalAvailPNC, 
  ...
  if (MJobNLDistribute(J,tmpMNL,MNodeList) == FAILURE)
    ...
    DBG(4,fSCHED)
      ...
      for (rqindex = 0;J->Req[rqindex] != NULL;rqindex++)
        ...
        DBG(4,fSCHED) DPrint("INFO:     tasks found for job %s:%d  (%3d required  %3d available)\n",
          J->Name,
          rqindex,
          RQ->TaskCount,
...
int MJobDistributeTaskGeometry(
  ...
  DBG(4,fSCHED) DPrint("%s(%s,RQ,NL,NI)\n",
    FName,
    (J != NULL) ? J->Name : "NULL");
...
*/
      ...
      if (MPar[0].NodeSetTolerance > 0.0)
        ...
        for (sindex = 0;sindex < MaxSet;sindex++)
          ...
          for (sindex2 = sindex + 1;sindex2 < MaxSet;sindex2++)
            {
            Distance = (double)(SetIndex[sindex2] - SetIndex[sindex]) / SetIndex[sindex];
            ...
            if (EffSetCount[sindex] < TasksRequired) 
              {
              MinSetLoss[sindex] += (SetIndex[sindex2] - SetIndex[sindex]) * 
                                    MIN(SetCount[sindex2],TasksRequired - EffSetCount[sindex]);
              }
  ...
  if (SetMembers < TasksRequired)
    {
    DBG(2,fSCHED) DPrint("INFO:     inadequate resources found in any set (%d < %d)\n",
      SetMembers,
      TasksRequired);
      ...
      for (sindex = 0;sindex < MaxSet;sindex++)
        ...
        switch (MPar[0].NodeSetPriorityType)
          ...
          case mrspBestFit:
            ...
            if ((SetValue == -1) || (EffSetCount[sindex] < SetValue)) 
              {
              BestSet  = sindex;
          ...
          case mrspMinLoss:
            ...
            if ((SetValue == -1) || (MinSetLoss[sindex] < SetValue))
              {
          ...
          case mrstProcSpeed: 
            ...
            if (MPar[0].NodeSetTolerance > 0.0)
              ...
              if (N->ProcSpeed >= SetIndex[BestSet])
                {
                Distance = (double)(SetIndex[BestSet] - SetIndex[sindex]) / SetIndex[sindex]; 
...
int MQueueGetBestRQTime(
  ...
  if (MQueueGetRequeueValue(AQ,MSched.Time,CTime,&tmpValue) != FAILURE)
    ...
    DBG(3,fSCHED) DPrint("INFO:     requeue value %.2lf found for immediate action (T: %s)\n",
      BestValue,
      MULToTString(0));
    }
  else
  ...
  for (jindex = 0;AQ[jindex] > 0;jindex++)
    ...
    if ((BestTime == -1) || (tmpValue > BestValue))
      ...
      DBG(3,fSCHED) DPrint("INFO:     requeue value %.2lf found at completion of job %s (T: %s)\n",
        BestValue,
        J->Name,
  ...
  switch(AIndex)
    ...
    case msaFBServer:
      ...
      if (S->FBServerHost[0] != '\0')
        {
        MUURLCreate(NULL,S->FBServerHost,NULL,S->FBServerPort,Buf,MAX_MLINE);
        }
    ...
    case msaServer:
      ...
      MUURLCreate(NULL,S->ServerHost,NULL,S->ServerPort,Buf,MAX_MLINE);
...
int MSchedSetDefaults(
  ...
  if ((ptr = getenv(MSCHED_ENVHOMEVAR)) != NULL)
    ...
    if ((buf = MFULoad(MASTER_CONFIGFILE,1,macmRead,&count,NULL)) != NULL)
      {
  ...
  if (!strstr(S->HomeDir,DEFAULT_SchedLOCKFILE))
    {
    sprintf(S->LockFile,"%s%s",
      S->HomeDir,
      DEFAULT_SchedLOCKFILE);
    }
  else
    {
  ...
  if (!strstr(S->HomeDir,DEFAULT_SchedCONFIGFILE))
    {
    sprintf(S->ConfigFile,"%s%s",
      S->HomeDir,
      DEFAULT_SchedCONFIGFILE);
    }
  else
    {
...
int MSchedToString(
  ...
  for (aindex = 0;CPAList[aindex] != -1;aindex++)
    ...
    MXMLSetAttr(E,(char *)MSchedAttr[CPAList[aindex]],tmpString,mdfString);    
    }    /* END for (aindex) */
...
int MSchedFromString(
  ...
  for (aindex = 0;CPAList[aindex] != -1;aindex++)
    ...
    if (MXMLGetAttr(
         E,
         (char *)MSchedAttr[CPAList[aindex]],
         NULL,
         tmpString,
...
int MResAllocateRE(
  ...
  DBG(3,fUI) DPrint("%s(%s,%d,%d,NodeList,TaskCount,%ld,%ld,MsgBuf,%d,DRes)\n",
    FName,
    PBuf,
    PBufSize,
    PIndex,
  ...
  if ((ptr = strstr(PBuf,"TASKS")) != NULL)
    ...
    if ((cindex != mcmpEQ) && (cindex != mcmpGE))
      ...
      if (MPtr != NULL)
        {
        MUSNPrintF(&MPtr,&MSpace,"ERROR:    cannot parse expression '%s'\n",
          PBuf);
        }
  ...
  DBG(3,fUI) DPrint("INFO:     setres requesting 'TASKS %s %d'\n",
    MComp[cindex],
    TaskReqCount);
  ...
  if (MJobGetEStartTime(
    ...
    if (BestTime > StartTime)
      ...
      if (MPtr != NULL)
        {
        MUSNPrintF(&MPtr,&MSpace,"ERROR:    cannot select %d tasks for reservation for %s\n",
          TaskReqCount,
    ...
    if (MPtr != NULL)
      {
      MUSNPrintF(&MPtr,&MSpace,"ERROR:    only %d tasks/%d nodes available for reservation\n",
        tmpTaskCount,
        tmpNodeCount);
      }
  ...
  for (index = 0;index < tmpNodeCount;index++)
    ...
    if (BSpace < 100)
      ...
      MUSNPrintF(&MPtr,&MSpace,"ERROR:    cannot maintain regex for reservation\n"); 
  ...
  switch (PIndex)
    ...
    case mcoComputeHosts:
      ...
      for (index = 0;MAX_MNODE;index++)
        ...
        DBG(3,fCONFIG) DPrint("INFO:     compute host '%s' added\n",
          SArray[index]);
        }  /* END for (index) */
      }  /* END BLOCK */
    ...
    case pParIgnQList:
      ...
      if (S->IgnQList == NULL)
	S->IgnQList = (char **)calloc(1,sizeof(char *) * MAX_MCLASS);
    ...
    case pLogFacility:
      ...
      for (index = 0;SArray[index] != NULL;index++)
        ...
        for (vindex = 0;MLogFacilityType[vindex] != NULL;vindex++)
          {
          if (strstr(SArray[index],MLogFacilityType[vindex]) != NULL)
            {
    ...
    case pAdminEAction:
      ...
      if ((MFUGetInfo(tmpPathName,NULL,NULL,&IsExe) == FAILURE) ||
          (IsExe == FALSE))
        {
        DBG(2,fCONFIG) DPrint("ALERT:    invalid action program '%s' requested\n",
          tmpPathName);

        DBG(2,fCONFIG) DPrint("INFO:     disabling action program '%s'\n",
          S->Action[mactAdminEvent]);
    ...
    case mcoJobFBAction:
      ...
      if ((MFUGetInfo(tmpPathName,NULL,NULL,&IsExe) == FAILURE) ||
          (IsExe == FALSE))
        {
        DBG(2,fCONFIG) DPrint("ALERT:    invalid job FB program '%s' requested\n",
          tmpPathName);

        DBG(2,fCONFIG) DPrint("INFO:     disabling job FB program '%s'\n",
          S->Action[mactJobFB]);
    ...
    case pResCtlPolicy:
      ...
      S->ResCtlPolicy = MUGetIndex(SVal,(const char **)MResCtlPolicy,FALSE,0);

      break;

    case mcoResLimitPolicy:

      S->ResLimitPolicy = MUGetIndex(SVal,(const char **)MPolicyMode,FALSE,0);
    ...
    case mcoAdmin1Users:
      ...
      for (aindex = 0;SArray[aindex] != NULL;aindex++)
        ...
        if (aindex == MAX_MADMINUSERS)
          {
          DBG(0,fCONFIG) DPrint("ALERT:    max admin users (%d) reached\n",
            MAX_MADMINUSERS);
        ...
        while (ptr != NULL)
          {
          MUStrCpy(S->Admin1User[aindex],ptr,sizeof(S->Admin1User[aindex]));
    ...
    case mcoAdmin2Users:
      ...
      for (aindex = 0;SArray[aindex] != NULL;aindex++)
        ...
        while (ptr != NULL)
          {
          MUStrCpy(S->Admin2User[aindex],ptr,sizeof(S->Admin2User[aindex]));
    ...
    case mcoAdmin3Users:
      ...
      for (aindex = 0;SArray[aindex] != NULL;aindex++)
        ...
        while (ptr != NULL)
          {
          MUStrCpy(S->Admin3User[aindex],ptr,sizeof(S->Admin3User[aindex]));
    ...
    case mcoAdmin4Users:
      ...
      for (aindex = 0;SArray[aindex] != NULL;aindex++)
        ...
        while (ptr != NULL)
          {
          MUStrCpy(S->Admin4User[aindex],ptr,sizeof(S->Admin4User[aindex]));

          ptr = MUStrTok(NULL,", \t",&TokPtr);
          }  /* END while (ptr != NULL) */

        DBG(3,fCONFIG) DPrint("INFO:     ADMIN4 '%s' added\n",
          S->Admin4User[aindex]);
        }    /* END for (aindex) */
    ...
    case mcoAdminHosts:
      ...
      for (aindex = 0;SArray[aindex] != NULL;aindex++)
        ...
        if (aindex == MAX_MADMINHOSTS)
          {
          DBG(0,fCONFIG) DPrint("ALERT:    max admin hosts (%d) reached\n",
            MAX_MADMINHOSTS);

          break;
          }

        MUStrCpy(S->AdminHost[aindex],SArray[aindex],sizeof(S->AdminHost[aindex]));

        DBG(3,fCONFIG) DPrint("INFO:     admin host '%s' added\n",
          S->AdminHost[aindex]);
        }    /* END for (aindex) */
    ...
    case pServerHost:
      ...
      if ((strncmp(HostName,S->ServerHost,strlen(S->ServerHost))) &&
        ...
        fprintf(stderr,"ERROR:    server must be started on host '%s' (currently on '%s')\n",
          S->ServerHost,
          HostName);
        ...
        {
        DBG(2,fCONFIG) DPrint("INFO:     starting scheduler on '%s'\n",
          S->ServerHost);
        }
      }    /* END BLOCK */
    ...
    case pMonitoredFunction:
      ...
      MUStrCpy(
        S->MonitoredFunction,
        SVal,
        sizeof(S->MonitoredFunction));

      break;

    case pPreemptPolicy:

      S->PreemptPolicy = MUGetIndex(SVal,(const char **)MPreemptPolicy,0,0);
    ...
    case pSchedLogFile:
      ...
      DBG(0,fALL) DPrint("INFO:     starting %s version %s ##################\n",
        MSCHED_NAME,
        MSCHED_VERSION);
    ...
    case pNAPolicy:
      ...
      S->DefaultNAccessPolicy = 
        (enum MNodeAccessPolicyEnum)MUGetIndex(SVal,MNAccessPolicy,FALSE,DEFAULT_MNACCESSPOLICY);
    ...
    case mcoWCViolAction:
      ...
      MSched.WCViolAction = MUGetIndex(SVal,MWCVAction,FALSE,MSched.WCViolAction);
...
int MJobNLDistribute(
  ...
  DBG(4,fSCHED) DPrint("%s(%s,SrcMNL,DstMNL)\n",
    FName,
    (J != NULL) ? J->Name : "NULL");
  ...
  for (rqindex = 0;J->Req[rqindex] != NULL;rqindex++)
    ...
    for (nindex = 0;SrcMNL[rqindex][nindex].N != NULL;nindex++)
      ...
      DBG(8,fSCHED) DPrint("INFO:     job %s:%d nodelist[%d]:  %sx%d\n",
        J->Name,
        rqindex,
        nindex,
        SrcMNL[rqindex][nindex].N->Name,
        SrcMNL[rqindex][nindex].TC);
      }  /* END for (nindex) */

    DBG(6,fSCHED) DPrint("INFO:     job %s:%d nodes:  %d\n",
      J->Name,
      rqindex,
      nindex);
    }  /* END for (rqindex) */
  ...
  for (rqindex = 0;J->Req[rqindex] != NULL;rqindex++)
    ...
    DBG(6,fSCHED) DPrint("INFO:     job %s:%d avail:  %d\n",
      J->Name,
      rqindex,
      nindex);
    }  /* END for (rqindex) */
    ...
    for (nindex = 0;NodeList[nindex].N != NULL;nindex++)
      ...
      for (index = 0;J->Req[index] != NULL;index++)
        ...
        if (NFreqSet[index][NodeList[nindex].N->Index] == TRUE)
          {
          TAvail[index] -= (double)TC / NFreq[NodeList[nindex].N->Index];
          }
        }    /* END for (index) */
  ...
  for (rqindex = 0;J->Req[rqindex] != NULL;rqindex++)
    ...
    for (nindex = 0;DstMNL[rqindex][nindex].N != NULL;nindex++)
      {
      DBG(7,fSCHED) DPrint("INFO:     job %s:%d nodelist[%d]: %sx%d\n",
        J->Name,
        rqindex,
        nindex,
        DstMNL[rqindex][nindex].N->Name,
    ...
    if (TReq[rqindex] > 0.0)
      ...
      {
      DBG(4,fSCHED) DPrint("INFO:     dist nodelist for job %s:%d (%d of %d)\n",
        J->Name,
        rqindex,
        NIndex[rqindex],
...
int MJobDistributeTasks(
  ...
  DBG(3,fSCHED) DPrint("%s(%s,%s,NodeList,TaskMap)\n",
    FName,
    (J != NULL) ? J->Name : "NULL",
    (R != NULL) ? R->Name : "NULL");
  ...
  for (rqindex = 0;J->Req[rqindex] != NULL;rqindex++)
    ...
    if (J->Geometry != NULL)
      {
      if (MJobDistributeTaskGeometry(J,RQ,NodeList,&index) == SUCCESS)
        {
        continue;
        }
      }
    ...
    if (R->Type == mrmtLL)
      ...
      if (RQ->NodeCount > 0)
        ...
        if (TasksAvail < TasksRequired)
          {
          DBG(2,fSCHED) DPrint("ALERT:    inadequate tasks found on %d nodes in DistributeTasks() (%d < %d)\n",
            AllocIndex,
            TasksAvail,
            TasksRequired);
      ...
      for (nindex = 0;RQ->NodeList[nindex].N != NULL;nindex++)
        ...
        if (RQ->NodeCount > 0)
          ...
          if (AttemptBalancedDistribution == TRUE)
            ...
            if ((RQ->NodeList[nindex].TC > TPN) && (Overflow > 0))
              {
              tmpTaskCount = MIN(RQ->NodeList[nindex].TC - TPN,Overflow);
          ...
          RQ->NodeList[nindex].TC = MIN(MaxTPN,RQ->NodeList[nindex].TC);
          }
        ...
        RQ->NodeList[nindex].TC =
          (short)MIN(RQ->TaskCount - taskcount,RQ->NodeList[nindex].TC);
      ...
      if (index <= 0)
        {
        DBG(2,fSCHED) DPrint("ALERT:    inadequate tasks allocated to job\n");
  ...
  switch(Distribution)
    ...
    default:
      ...
      DBG(4,fSCHED) DPrint("ERROR:    unexpected task distribution (%d) in %s()\n",
        Distribution,
        FName);
  ...
  if ((J->Req[1] != NULL) && (NodeList != NULL))
    ...
    for (nindex1 = 0;NodeList[nindex1].N != NULL;nindex1++)
      ...
      for (nindex2 = nindex1 + 1;NodeList[nindex2].N != NULL;nindex2++)
        {
        if (NodeList[nindex2].N != NodeList[nindex3].N)
          continue;
...
int MSchedUpdateStats()
  ...
  MStat.TotalPHBusy        += (double)(GP->URes.Procs - GP->ARes.Procs) *
                               SchedTime;
